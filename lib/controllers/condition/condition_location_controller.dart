...
class ConditionLocationController extends GetxController {
  final _selectedLocation = <String>{}.obs;
  final _buttonColors = <String, Color>{}.obs; // 버튼 색상 상태 관리 변수

  Set<String> get selectedBuildingTypes => _selectedLocation.value;
  Map<String, Color> get buttonColors => _buttonColors.value; // 게터 추가

  void toggleSelection(String buildingType) {
    if (_selectedLocation.contains(buildingType)) {
      _selectedLocation.remove(buildingType);
      _buttonColors[buildingType] = Colors.grey; // 선택 해제 시 버튼 색상 변경
    } else {
      _selectedLocation.add(buildingType);
